...
  const username = localStorage.getItem('username')
  const userOrders = JSON.parse(localStorage.getItem(`${username}orders`)) || []
  const allOrders = JSON.parse(localStorage.getItem('Orders')) || []
...
  // ✅ Load products from correct key
  const allProducts = JSON.parse(localStorage.getItem('productsList')) || []

  // ✅ Update stock count in productsList
  cancelledItems.forEach((cancelledItem) => {
    const index = allProducts.findIndex((p) => String(p.id) === String(cancelledItem.id))
    if (index !== -1) {
...
  // ✅ Remove order from user and global order list
  const updatedUserOrders = userOrders.filter((order) => order[0] !== orderId)
  const updatedGlobalOrders = allOrders.filter((order) => order[0] !== orderId)
...
  const productDetailsString = cancelledItems.map(item => {
...
  }).catch(err => console.error("User cancel email failed", err));
